...
A [Lethal Company][lethal-company-steam] mod that scans the number of scraps on
the moon and sends it to the chat at the start of each day, just like the
...
Create BepInEx profiles in `./profiles` if you use the local debug helper.

```powershell
pwsh ./InitProfiles.ps1
```

Open `AutoTerminalScanClassic.sln` in Visual Studio.

Launch the `HostAndGuest` profile to debug, or manually run `Debug.ps1`.
...
- `actionlint` checks workflow syntax, expressions, runner labels, and
  composite action metadata.

Run ShellCheck before actionlint. This keeps actionlint's ShellCheck integration
available so actionlint can also inspect inline shell scripts in workflows.
The pyflakes integration remains disabled because this repository does not
currently contain Python files. Revisit that setting if Python scripts are
added.
...
4. Verify Thunderstore package metadata in `assets/manifest.json`:
    ...
    - Confirm the manifest description still matches the Thunderstore-facing
      release intent.
5. Replace version in `AutoTerminalScanClassic/AutoTerminalScanClassic.csproj`
   with a SemVer version such as `1.2.3`.
...
from `assets/README.md`. List slash-separated versions from older to newer.
...
`assets/README.md` and `CHANGELOG.md`. Treat `CHANGELOG.md` as the
developer-facing compatibility history.

Handle dependency string changes in `assets/manifest.json` as separate
dependency maintenance. Document the reason and compatibility impact in that
change.

### Thunderstore publishing

The current workflow deploys to the Thunderstore `aoirint` team and publishes
to the [`lethal-company`][thunderstore-lethal-company-community] community with
these categories:
